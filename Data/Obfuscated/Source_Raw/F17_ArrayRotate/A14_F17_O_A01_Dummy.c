/**
 * LGL-DynT4 Dataset - Function: ArrayRotate
 * -----------------------------------------
 * File: A14_F17_O_A01_Dummy.c
 * Logic: Reversal with dummy junk loops.
 */
#include <stdio.h>
#include <stdlib.h>

void c(int u42S[], int Y, int d6) {
    if (Y == 0) return;
    d6 %= Y;
    int dtNO[3][2] = {{0, d6-1}, {d6, Y-1}, {0, Y-1}};
    for(int egRd=0; egRd<3; egRd++) {
        int zgNW = dtNO[egRd][0], o3MY = dtNO[egRd][1];
        while(zgNW < o3MY) {
            for(int D=0; D<1; D++) {
                int DYB4 = u42S[zgNW]; u42S[zgNW] = u42S[o3MY]; u42S[o3MY] = DYB4;
            }
            zgNW++; o3MY--;
        }
    }
}

int main(int v, char *Lmg[]) {
    if (v < 3) return 0;
    int d6 = atoi(Lmg[1]);
    int Y = v - 2;
    int *u42S = (int*)malloc(Y * sizeof(int));
    for(int egRd=0; egRd<Y; egRd++) u42S[egRd] = atoi(Lmg[egRd+2]);
    c(u42S, Y, d6);
    for(int egRd=0; egRd<Y; egRd++) printf("%d ", u42S[egRd]);
    printf("\n");
    free(u42S);
    return 0;
}